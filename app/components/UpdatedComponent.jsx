...
const UpdatedComponent = ({ updatedData }) => {
  ...
  useEffect(() => {
    if (updatedData) {
      setFormData((prev) => ({
        ...prev,
        ...updatedData,
        images: updatedData.images?.length ? updatedData.images : Array(10).fill(""),
      }));
    }
  }, [updatedData]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URL}api/add-id/${id}`,
        formData
      );
    ...
    <div className="min-h-screen bg-gray-200 p-4 md:p-8">
      <div className="">
        <div className="mb-8 border-b-4 border-black pb-4">
          <h1 className="text-4xl md:text-5xl font-bold text-black">EDIT GAME ID</h1>
          <p className="text-gray-600 mt-2">FEdit your game ID information to ensure all details are accurate and up to date!</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center pt-4">
            ...
            >
              {uploading ? "Uploading..." : "UPDATED ID"}
            </button>
          </div>
        </div>
      </div>
